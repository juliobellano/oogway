...
// DOM element cache
const elements = {};
const NO_ACTION_TOKEN = "NO_ACTION";
...
function initDOM() {
  const ids = [
    ...
    "startVideoBtn",
    "startScreenBtn",
    "toggleProactiveCheckBtn",
    "proactiveRule",
    "proactiveCheckStatus",
    "videoPreview",
    "micSelect",
    "cameraSelect",
    "volume",
...
function isNoActionToken(text) {
  if (typeof text !== "string") return false;
  const normalized = text.trim().replace(/^["'`\s]+|["'`\s.?!,:;]+$/g, "");
  return normalized.toUpperCase() === NO_ACTION_TOKEN;
}

function setProactiveCheckStatus(text) {
  if (elements.proactiveCheckStatus) {
    elements.proactiveCheckStatus.textContent = text;
  }
}

function resetProactiveTurnState() {
  state.proactiveCheck.inFlight = false;
  state.proactiveCheck.pendingTurn = false;
  state.proactiveCheck.suppressCurrentTurn = false;
  state.proactiveCheck.transcriptBuffer = "";
  state.proactiveCheck.turnHadAssistantOutput = false;
}

function stopProactiveCheck(reason = "idle") {
  if (state.proactiveCheck.timerId) {
    clearInterval(state.proactiveCheck.timerId);
    state.proactiveCheck.timerId = null;
  }

  state.proactiveCheck.enabled = false;
  state.proactiveCheck.cooldownUntil = 0;
  resetProactiveTurnState();

  if (elements.toggleProactiveCheckBtn) {
    elements.toggleProactiveCheckBtn.textContent = "Start Proactive Check";
  }
  setProactiveCheckStatus(reason);
}

function buildProactiveCheckPrompt(rule) {
  return [
    "[VISION_CHECK]",
    `Rule: ${rule}`,
    "Evaluate only the most recent camera frame I just sent.",
    "If the rule is met, respond with one short user-facing sentence.",
    `If the rule is not met, respond exactly ${NO_ACTION_TOKEN}.`,
    `When returning ${NO_ACTION_TOKEN}, do not produce spoken audio.`,
  ].join("\n");
}

async function runProactiveCheckTick() {
  if (!state.proactiveCheck.enabled) return;
  if (!state.client || !state.client.connected) {
    stopProactiveCheck("disconnected");
    return;
  }
  if (!state.video.isStreaming) {
    stopProactiveCheck("camera required");
    addMessage("[Proactive check stopped: camera is off]", "system");
    return;
  }
  if (state.proactiveCheck.inFlight) {
    setProactiveCheckStatus("waiting for turn");
    return;
  }

  const now = Date.now();
  if (now < state.proactiveCheck.cooldownUntil) {
    const secondsLeft = Math.max(
      1,
      Math.ceil((state.proactiveCheck.cooldownUntil - now) / 1000)
    );
    setProactiveCheckStatus(`cooldown (${secondsLeft}s)`);
    return;
  }

  const rule = (elements.proactiveRule?.value || "").trim();
  if (!rule) {
    stopProactiveCheck("rule required");
    addMessage("[Proactive check stopped: add a proactive rule]", "system");
    return;
  }

  const frameBase64 = capturePreviewFrameBase64();
  if (!frameBase64) {
    setProactiveCheckStatus("waiting for video frame");
    return;
  }

  state.proactiveCheck.inFlight = true;
  state.proactiveCheck.pendingTurn = true;
  state.proactiveCheck.suppressCurrentTurn = false;
  state.proactiveCheck.transcriptBuffer = "";
  state.proactiveCheck.turnHadAssistantOutput = false;
  setProactiveCheckStatus("running");

  try {
    await state.client.sendImageMessage(frameBase64, "image/jpeg");
    state.client.sendTextMessage(buildProactiveCheckPrompt(rule));
  } catch (error) {
    console.error("Proactive check tick failed:", error);
    resetProactiveTurnState();
    setProactiveCheckStatus("error");
  }
}
...
function startProactiveCheck() {
  ...
  if (!state.video.isStreaming) {
    addMessage("[Turn on camera first]", "system");
    return;
  }
  if (!(elements.proactiveRule?.value || "").trim()) {
    addMessage("[Add a proactive rule first]", "system");
    return;
  }
  ...
  state.proactiveCheck.enabled = true;
  state.proactiveCheck.cooldownUntil = 0;

  if (elements.toggleProactiveCheckBtn) {
    elements.toggleProactiveCheckBtn.textContent = "Stop Proactive Check";
  }

  setProactiveCheckStatus("running");
  runProactiveCheckTick();
  state.proactiveCheck.timerId = setInterval(
    runProactiveCheckTick,
    state.proactiveCheck.intervalMs
  );
  addMessage("[Proactive check on]", "system");
}

function toggleProactiveCheck() {
  if (state.proactiveCheck.enabled) {
    stopProactiveCheck("idle");
    addMessage("[Proactive check off]", "system");
    return;
  }
...
async function connect() {
  ...
  try {
    ...
    if (!isGroundingEnabled) {
      ...
      // Add dots tool if enabled
      if (elements.enableDotsTool.checked) {
        const dotsTool = new DotsTool();
        state.client.addFunction(dotsTool);
        console.log("✅ Dots tool enabled");
      }

      // Add locate element tool if enabled
      if (elements.enableLocateTool.checked) {
        const locateTool = new LocateElementTool();
        state.client.addFunction(locateTool);
        console.log("✅ Locate element tool enabled");
      }
    } else {
      console.log(
...
function handleMessage(message) {
  ...
  switch (message.type) {
    case MultimodalLiveResponseType.TEXT:
      ...
      if (
        state.proactiveCheck.pendingTurn &&
        isNoActionToken(String(message.data || ""))
      ) {
        state.proactiveCheck.suppressCurrentTurn = true;
        break;
      }
      if (
        state.proactiveCheck.pendingTurn &&
        state.proactiveCheck.suppressCurrentTurn
      ) {
        break;
      }
      if (state.proactiveCheck.pendingTurn) {
        state.proactiveCheck.turnHadAssistantOutput = true;
      }
    ...
    case MultimodalLiveResponseType.OUTPUT_TRANSCRIPTION:
      ...
      if (state.proactiveCheck.pendingTurn) {
        state.proactiveCheck.transcriptBuffer += message.data.text || "";
        if (isNoActionToken(state.proactiveCheck.transcriptBuffer)) {
          state.proactiveCheck.suppressCurrentTurn = true;
          if (state.audio.player) state.audio.player.interrupt();
          break;
        }
        if (state.proactiveCheck.suppressCurrentTurn) {
          break;
        }
        state.proactiveCheck.turnHadAssistantOutput = true;
      }
    ...
    case MultimodalLiveResponseType.TOOL_CALL:
      ...
      for (let index = 0; index < functionCalls.length; index++) {
        executeToolCall(functionCalls[index]);
      }
      break;
    ...
    case MultimodalLiveResponseType.TURN_COMPLETE:
      ...
      if (state.proactiveCheck.pendingTurn) {
        if (
          !state.proactiveCheck.suppressCurrentTurn &&
          state.proactiveCheck.turnHadAssistantOutput
        ) {
          state.proactiveCheck.cooldownUntil =
            Date.now() + state.proactiveCheck.cooldownMs;
          setProactiveCheckStatus("cooldown");
        } else if (state.proactiveCheck.enabled) {
          setProactiveCheckStatus("running");
        }
        resetProactiveTurnState();
      }
...
async function executeToolCall(functionCall) {
  ...
  try {
    let result = state.client.callFunction(functionName, parameters);
    if (result && typeof result.then === "function") {
      result = await result;
    }

    if (functionId) {
      state.client.sendToolResponse(functionId, {
        status: "ok",
        result: result || {},
      });
    }
...
function extractLocateIntent(message) {
  const text = (message || "").trim();
  const whereMatch = text.match(/^where is\s+(.+?)\??$/i);
  if (whereMatch) return whereMatch[1].replace(/^the\s+/i, "").trim();

  const findMatch = text.match(/^(find|locate)\s+(.+?)\??$/i);
  if (findMatch) return findMatch[2].replace(/^the\s+/i, "").trim();

  return null;
}

function capturePreviewFrameBase64() {
...
async function requestLocateBoxesFromGemini3(query, imageBase64) {
  ...
  if (!response.ok) {
    throw new Error(payload.error || "Failed to locate element");
  }
  return Array.isArray(payload.boxes) ? payload.boxes : [];
}

async function tryLocateAndHighlightElementWithGemini3(message) {
  const query = extractLocateIntent(message);
  if (!query || !state.client) return false;

  const imageBase64 = capturePreviewFrameBase64();
  if (!imageBase64) {
    addMessage(
      "[Turn on screen sharing so Gemini-3 Pro can inspect the screen]",
      "system"
    );
    return true;
  }
  ...
  try {
    ...
    if (boxes.length === 0) {
      addMessage("[Gemini-3 Pro could not find that element]", "system");
      return true;
    }

    let result = state.client.callFunction("dots_tool", {
    ...
    console.error("Failed to render dots_tool highlight:", error);
    addMessage("[Dots tool unavailable]", "system");
  }
...
async function toggleVideo() {
  if (!state.video.isStreaming) {
    ...
    if (state.proactiveCheck.enabled) {
      stopProactiveCheck("camera required");
      addMessage("[Proactive check stopped: camera turned off]", "system");
    }
    if (state.video.streamer) state.video.streamer.stop();
    state.video.isStreaming = false;
...
async function sendMessage() {
  ...
  if (state.client) {
    ...
    const handledLocally = await tryLocateAndHighlightElementWithGemini3(message);
    if (handledLocally) {
...
function initEventListeners() {
  ...
  elements.toggleProactiveCheckBtn.addEventListener("click", toggleProactiveCheck);
